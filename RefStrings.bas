...
End Function
'конвертаци€ байтового массива без изменени€ его размера (UCase, LCase)
...
Sub LCaseBufInt(IntStr() As Integer)
    ...
    Next
End Sub
'јналог UCase дл€ байтового массива
...
'аналог InStr$() с дополнителным параметром endFind, чтобы указывать позицию окончани€ поиска.
